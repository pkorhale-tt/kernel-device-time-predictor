...
# TT-Metal proven hardware constant
# source: matmul_device_operation.cpp line 1344
# "The Wormhole/Blackhole matrix engine performs 8x16 x 16x16 = 8x16
#  in a single cycle. This is 2*8*16*16 = 4096 muladds in a single cycle."
TENSIX_MACS_PER_CYCLE_LOFI = 4096
TILE_MACS                  = TILE * TILE * 2        # 32×32×2 = 2,048 MACs per tile
CYCLES_PER_TILE_LOFI       = TILE_MACS / TENSIX_MACS_PER_CYCLE_LOFI  # = 0.5

OVERHEAD_PER_TILE = 40
BIG_OP_FACTOR     = 2.0

FIDELITY = {"LoFi": 1, "HiFi2": 2, "HiFi3": 3, "HiFi4": 4}

# ── Effective BW table (GB/s) ──────────────────────────────────────────────────
# Calibrated from Wormhole silicon measurements.
# Original values — not changed.
BW_TABLE = {
    "1D":      {"small": 1.2e9,   "medium": 1.9e9,   "large": 4.0e9},
    "2D":      {"small": 1.155e9, "medium": 1.563e9, "large": 3.5e9},
    "REUSE":   {"small": 1.0e9,   "medium": 1.5e9,   "large": 3.0e9},
    "DEFAULT": {"small": 1.0e9,   "medium": 1.5e9,   "large": 3.0e9},
}
BW_THRESHOLDS = [500_000, 5_000_000]
...
def predict_time(op: dict) -> dict:
    ...
    if not all([M, K, N]):
        result.update(
            predicted_ns         = None,
            bottleneck           = None,
            compute_cyc_per_k    = None,
            reader_cyc_per_k     = None,
            total_bytes_per_core = None,
            eff_bw_GBs           = None,
    ...
    tiles_C_out = per_core_M * per_core_N

    # ── Compute cycles per K-step ─────────────────────────────────────────────
    # Proven formula from matmul_device_operation.cpp line 1344:
    #   tensix_mul_adds_per_cycle_lofi = 4096
    #   CYCLES_PER_TILE_LOFI = 2048 / 4096 = 0.5
    #
    # compute_cyc = CYCLES_PER_TILE_LOFI x tiles_C_out x fidelity
    #             = 0.5 x tiles_C_out x fm
    #
    # Validated from PM COMPUTE back-calculation:
    #   PM_COMPUTE / total_steps / fidelity ≈ 7.0 for Q proj ✓
    compute_cyc = CYCLES_PER_TILE_LOFI * tiles_C_out * fm
    ...
    # ── Reader cycles per K-step ──────────────────────────────────────────────
    eff_bw     = get_eff_bw(total, cs)
    tiles_C_pc = tiles_C_out
    ...
    result.update(
        ...
        compute_cyc_per_k    = round(compute_cyc, 3),
...
def main():
    p = argparse.ArgumentParser(
        description="Step 3 — predict kernel time per op")
    p.add_argument("configured_yaml",
                   help="Input: ops_configured.yaml from step2")
    ...
    for o in out:
        ...
        bw  = o.get("eff_bw_GBs", "")
        print(f"{o['op_id']:>4}  {str(ns or 'N/A'):>10}  "
              f"{bot:>9}  {str(bw):>5}  "
              f"{o['description'][:40]}")
    print("─" * 78)
