...
#models/demos/vision/classification/vit/
import ttnn
...
def test_vit_patch_embeddings(device, model_name, batch_size, image_size, image_channels, model_location_generator):
    ...
    torch_output, *_ = model.vit.embeddings.patch_embeddings(torch_pixel_values.to(torch.float32))
...
def test_vit_embeddings(device, model_name, batch_size, image_size, image_channels, model_location_generator):
    ...
    dataset = load_dataset("huggingface/cats-image", split="train")
    ...
    torch_output, *_ = model.vit.embeddings(torch_pixel_values.to(torch.float32))
...
def test_vit_attention(device, model_name, batch_size, sequence_size):
    ...
    with open('modeldump-attention.txt', 'w') as fout:
        print(f'{config=}', file=fout)
    ...
    with open('paramdump-attention.txt', 'w') as fout:
        print(f'{parameters}', file=fout)
...
def test_vit_intermediate(device, model_name, batch_size, sequence_size, torch_dtype):
    ...
    config = transformers.ViTConfig.from_pretrained(model_name)
    with open('modeldump-intermediate.txt', 'w') as fout:
        print(f'{config=}', file=fout)
    ...
    with open('paramdump-intermediate.txt', 'w') as fout:
        print(f'{parameters}', file=fout)
    hidden_states = ttnn.from_torch(torch_hidden_states, layout=ttnn.TILE_LAYOUT, device=device)
...
def test_vit_output(device, model_name, batch_size, sequence_size):
    ...
    config = transformers.ViTConfig.from_pretrained(model_name)
    with open('modeldump-output.txt', 'w') as fout:
        print(f'{config=}', file=fout)
    ...
    with open('paramdump-output.txt', 'w') as fout:
        print(f'{parameters}', file=fout)
...
def test_vit_layer(device, model_name, batch_size, sequence_size, model_location_generator):
    ...
    torch_attention_mask = torch.ones(1, sequence_size, dtype=torch.bfloat16)
    torch_output, *_ = model(torch_hidden_states.to(torch.float32), torch_attention_mask.to(torch.float32))
    ...
    assert_with_pcc(torch_output, output, 0.9998)  # 0.9957
...
def test_vit_encoder(device, model_name, batch_size, sequence_size, model_location_generator):
    ...
    torch_output = model(torch_hidden_states.to(torch.float32), torch_attention_mask).last_hidden_state
    ...
    assert_with_pcc(torch_output, output, 0.99)  # 0.9294
...
@pytest.mark.parametrize("image_size", [224])
@pytest.mark.parametrize("image_channels", [3])
def test_vit(device, model_name, batch_size, image_size, image_channels, model_location_generator):
    torch.manual_seed(0)
    ...
    image = dataset["image"][0:batch_size]
    image_processor = AutoImageProcessor.from_pretrained("google/vit-base-patch16-224")
    torch_pixel_values = image_processor(image, return_tensors="pt").pixel_values.to(torch.bfloat16)
    torch_pixel_values = torch_pixel_values.repeat(batch_size, 1, 1, 1)
